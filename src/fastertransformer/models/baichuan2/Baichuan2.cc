...
template<typename T>
void Baichuan2<T>::initialize()
{
    gpt_context_decoder_ = new Baichuan2ContextDecoder<T>(head_num_,
                                                      size_per_head_,
                                                      inter_size_,
                                                      num_layer_,
    ...
    gpt_decoder_ = new Baichuan2Decoder<T>(head_num_,
                                       size_per_head_,
                                       inter_size_,
                                       num_layer_,
                                       rotary_embedding_dim_,
    ...
    // parse env overrides
    if (std::getenv("BAICHUAN2_STREAM_CB_STEP") != nullptr) {
        try {
            int callback_step_from_env = stoi(
                std::string(std::getenv("BAICHUAN2_STREAM_CB_STEP"))
                );
            token_generated_cb_step_ = callback_step_from_env;
            FT_LOG_INFO("Override stream callback step to %d from BAICHUAN2_STREAM_CB_STEP",
                token_generated_cb_step_);
        } catch (...) {
            FT_LOG_WARNING("convert BAICHUAN2_STREAM_CB_STEP err, use default value %d",
                token_generated_cb_step_);
        }
    }
}
...
template<typename T>
void Baichuan2<T>::allocateBuffer()
{
    FT_CHECK(false);
}
...
template<typename T>
Baichuan2<T>::Baichuan2(size_t                              head_num,
...
template<typename T>
Baichuan2<T>::Baichuan2(Baichuan2<T> const& gpt):
    BaseLayer(gpt),
...
template<typename T>
void Baichuan2<T>::registerCallback(callback_sig* fn, void* ctx)
{
...
template<typename T>
void Baichuan2<T>::unRegisterCallback()
{
...
template<typename T>
void Baichuan2<T>::forward(std::vector<Tensor>*       output_tensors,
                         const std::vector<Tensor>* input_tensors,
                         const Baichuan2Weight<T>*    gpt_weights)
{
    FT_CHECK(false);
}

template<typename T>
void Baichuan2<T>::forward(std::unordered_map<std::string, Tensor>*       output_tensors,
                         ...
                         const Baichuan2Weight<T>*                        gpt_weights)
{
    ...
    finished_buf_ = (bool*)(output_tensors->at("finished").data);
...
template<typename T>
void Baichuan2<T>::sendTensorsToFirstPipelineNode(std::unordered_map<std::string, Tensor>*       output_tensors,
                                                const std::unordered_map<std::string, Tensor>* input_tensors)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    if (pipeline_para_.world_size_ == 1) {
...
template<typename T>
void Baichuan2<T>::setOutputTensors(std::unordered_map<std::string, Tensor>*       output_tensors,
                                  const std::unordered_map<std::string, Tensor>* input_tensors,
...
template<typename T>
size_t Baichuan2<T>::getPipelineParallelRank()
{
    return pipeline_para_.rank_;
}

template<typename T>
size_t Baichuan2<T>::getPipelineParallelSize()
{
    return pipeline_para_.world_size_;
}

template<typename T>
size_t Baichuan2<T>::getTensorParallelRank()
{
    return tensor_para_.rank_;
}

template<typename T>
size_t Baichuan2<T>::getTensorParallelSize()
{
    return tensor_para_.world_size_;
}

template<typename T>
bool* Baichuan2<T>::getFinishBuffer()
{
    return finished_buf_;
}

template class Baichuan2<float>;
template class Baichuan2<half>;
#ifdef ENABLE_BF16
template class Baichuan2<__nv_bfloat16>;
#endif
}  // namespace fastertransformer
