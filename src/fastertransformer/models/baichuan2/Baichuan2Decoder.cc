...
#include "src/fastertransformer/models/baichuan2/Baichuan2Decoder.h"
...
template<typename T>
void Baichuan2Decoder<T>::initialize()
{
...
template<typename T>
void Baichuan2Decoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void Baichuan2Decoder<T>::allocateBuffer(size_t batch_size)
{
...
template<typename T>
void Baichuan2Decoder<T>::freeBuffer()
{
...
template<typename T>
bool Baichuan2Decoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool Baichuan2Decoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool Baichuan2Decoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int Baichuan2Decoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
Baichuan2Decoder<T>::Baichuan2Decoder(size_t                              head_num,
...
template<typename T>
Baichuan2Decoder<T>::Baichuan2Decoder(Baichuan2Decoder<T> const& decoder):
...
template<typename T>
Baichuan2Decoder<T>::~Baichuan2Decoder()
{
...
template<typename T>
void Baichuan2Decoder<T>::forward(std::vector<Tensor>*                              output_tensors,
...
template<typename T>
void Baichuan2Decoder<T>::forward(std::unordered_map<std::string, Tensor>*          output_tensors,
...
template class Baichuan2Decoder<float>;
template class Baichuan2Decoder<half>;
#ifdef ENABLE_BF16
template class Baichuan2Decoder<__nv_bfloat16>;
#endif
