...
#include "src/fastertransformer/models/baichuan2/Baichuan2DecoderLayerWeight.h"
#include "src/fastertransformer/utils/memory_utils.h"

namespace fastertransformer {

template<typename T>
Baichuan2DecoderLayerWeight<T>::Baichuan2DecoderLayerWeight(const int  hidden_units,
    ...
    use_gptj_residual_(use_gptj_residual)
{
    ...
    FT_CHECK_WITH_INFO(int8_mode_ != 2, "Baichuan2 doesn't support int8_model == 2");
    FT_CHECK_WITH_INFO(!(std::is_same<T, float>::value && int8_mode_ == 1),
                       "Weight only quant does not work with FP32 compute.");
}

template<typename T>
Baichuan2DecoderLayerWeight<T>::Baichuan2DecoderLayerWeight(const int int8_mode): int8_mode_(int8_mode)
{
}

template<typename T>
Baichuan2DecoderLayerWeight<T>::~Baichuan2DecoderLayerWeight()
{
...
template<typename T>
void Baichuan2DecoderLayerWeight<T>::copyFrom(const Baichuan2DecoderLayerWeight& other)
{
...
template<typename T>
Baichuan2DecoderLayerWeight<T>::Baichuan2DecoderLayerWeight(const Baichuan2DecoderLayerWeight& other):
...
template<typename T>
Baichuan2DecoderLayerWeight<T>& Baichuan2DecoderLayerWeight<T>::operator=(const Baichuan2DecoderLayerWeight& other)
{
    hidden_units_      = other.hidden_units_;
    inter_size_        = other.inter_size_;
    tensor_para_size_  = other.tensor_para_size_;
...
template<typename T>
void Baichuan2DecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
...
template<typename T>
void Baichuan2DecoderLayerWeight<T>::setWeightPtr()
{
...
template<typename T>
void Baichuan2DecoderLayerWeight<T>::mallocWeights()
{
...
template struct Baichuan2DecoderLayerWeight<float>;
template struct Baichuan2DecoderLayerWeight<half>;
#ifdef ENABLE_BF16
template class Baichuan2DecoderLayerWeight<__nv_bfloat16>;
#endif
