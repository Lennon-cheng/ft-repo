...
#include "src/fastertransformer/models/baichuan2/Baichuan2ContextDecoder.h"
...
template<typename T>
void Baichuan2ContextDecoder<T>::initialize()
{
...
template<typename T>
void Baichuan2ContextDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void Baichuan2ContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool use_shared_contexts)
{
...
template<typename T>
void Baichuan2ContextDecoder<T>::freeBuffer()
{
...
template<typename T>
bool Baichuan2ContextDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool Baichuan2ContextDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool Baichuan2ContextDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int Baichuan2ContextDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
Baichuan2ContextDecoder<T>::Baichuan2ContextDecoder(size_t                              head_num,
...
template<typename T>
Baichuan2ContextDecoder<T>::Baichuan2ContextDecoder(Baichuan2ContextDecoder<T> const& decoder):
...
template<typename T>
Baichuan2ContextDecoder<T>::~Baichuan2ContextDecoder()
{
...
template<typename T>
void Baichuan2ContextDecoder<T>::forward(std::vector<Tensor>*                              output_tensors,
                                       const std::vector<Tensor>*                        input_tensors,
                                       const std::vector<Baichuan2DecoderLayerWeight<T>*>* gpt_decoder_layer_weight)
{
...
template<typename T>
void Baichuan2ContextDecoder<T>::forward(std::unordered_map<std::string, Tensor>*          output_tensors,
...
template class Baichuan2ContextDecoder<float>;
template class Baichuan2ContextDecoder<half>;
#ifdef ENABLE_BF16
template class Baichuan2ContextDecoder<__nv_bfloat16>;
#endif
