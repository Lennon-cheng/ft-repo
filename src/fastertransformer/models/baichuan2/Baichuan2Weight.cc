...
#include "src/fastertransformer/models/baichuan2/Baichuan2Weight.h"

namespace fastertransformer {

template<typename T>
Baichuan2Weight<T>::Baichuan2Weight(const int                                  hidden_units,
    ...
    prompt_learning_pair_(prompt_learning_pair)
{
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(new Baichuan2DecoderLayerWeight<T>(
            ...
            decoder_layer_weights.push_back(new Baichuan2DecoderLayerWeight<T>(0, 0));
        }
    }
...
template<typename T>
Baichuan2Weight<T>::~Baichuan2Weight()
{
...
template<typename T>
Baichuan2Weight<T>::Baichuan2Weight(const Baichuan2Weight& other):
...
template<typename T>
Baichuan2Weight<T>& Baichuan2Weight<T>::operator=(const Baichuan2Weight& other)
{
    hidden_units_               = other.hidden_units_;
    inter_size_                 = other.inter_size_;
    vocab_size_                 = other.vocab_size_;
...
template<typename T>
void Baichuan2Weight<T>::setWeightPtr()
{
...
template<typename T>
void Baichuan2Weight<T>::mallocWeights()
{
...
template<typename T>
void Baichuan2Weight<T>::loadModel(std::string dir_path)
{
    FtCudaDataType model_file_type = getModelFileType(dir_path + "/config.ini", "baichuan2");
...
template<typename T>
void Baichuan2Weight<T>::resizeLayer(const int num_layer)
{
    num_layer_ = num_layer;
    decoder_layer_weights.reserve(num_layer_);
    for (int l = 0; l < num_layer_; l++) {
...
template<typename T>
bool Baichuan2Weight<T>::isValidLayerParallelId(int l)
{
...
template struct Baichuan2Weight<float>;
template struct Baichuan2Weight<half>;
#ifdef ENABLE_BF16
template class Baichuan2Weight<__nv_bfloat16>;
#endif
