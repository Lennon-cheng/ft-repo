...
{
    Baichuan2TritonModelInstance<T>* model    = reinterpret_cast<Baichuan2TritonModelInstance<T>*>(ctx);
    auto                           result = Baichuan2TritonModelInstance<T>::convert_outputs(*output_tensors);

    model->stream_cb_(result, model->stream_ctx_);
}

template<typename T>
Baichuan2TritonModelInstance<T>::Baichuan2TritonModelInstance(
    std::unique_ptr<ft::Baichuan2<T>>                           gpt,
    std::shared_ptr<ft::Baichuan2Weight<T>>                     gpt_weight,
...
template<typename T>
std::unordered_map<std::string, ft::Tensor> Baichuan2TritonModelInstance<T>::convert_inputs(
...
Baichuan2TritonModelInstance<T>::convert_outputs(const std::unordered_map<std::string, ft::Tensor>& output_tensors)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
Baichuan2TritonModelInstance<T>::forward(std::shared_ptr<std::vector<triton::Tensor>> input_tensors)
{
    ft::FT_CHECK(false);
    return nullptr;
}
...
template<typename T>
std::shared_ptr<std::unordered_map<std::string, triton::Tensor>>
Baichuan2TritonModelInstance<T>::forward(std::shared_ptr<std::unordered_map<std::string, triton::Tensor>> input_tensors)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void Baichuan2TritonModelInstance<T>::allocateBuffer(const size_t request_batch_size,
                                                   ...
                                                   const size_t max_request_output_len)
{
    ...
    d_finished_ = (bool*)(allocator_->reMalloc(
        d_finished_, sizeof(bool) * request_batch_size * beam_width, false));
}

template<typename T>
void Baichuan2TritonModelInstance<T>::freeBuffer()
{
    ...
    allocator_->free((void**)(&d_finished_));
}

template struct Baichuan2TritonModelInstance<float>;
template struct Baichuan2TritonModelInstance<half>;
#ifdef ENABLE_BF16
template class Baichuan2TritonModelInstance<__nv_bfloat16>;
#endif
