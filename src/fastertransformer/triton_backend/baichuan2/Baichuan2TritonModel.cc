...
std::shared_ptr<AbstractTransformerModel> AbstractTransformerModel::createBaichuan2Model(std::string inifile)
{
    ...
    if (data_type == "half" || data_type == "fp16") {
        return std::make_shared<Baichuan2TritonModel<half>>(
        ...
        return std::make_shared<Baichuan2TritonModel<__nv_bfloat16>>(
            reader.GetInteger("ft_instance_hyperparameter", "tensor_para_size"),
            reader.GetInteger("ft_instance_hyperparameter", "pipeline_para_size"),
            reader.GetInteger("ft_instance_hyperparameter", "enable_custom_all_reduce", 0),
            model_dir,
        ...
        return std::make_shared<Baichuan2TritonModel<float>>(
            reader.GetInteger("ft_instance_hyperparameter", "tensor_para_size"),
            reader.GetInteger("ft_instance_hyperparameter", "pipeline_para_size"),
            reader.GetInteger("ft_instance_hyperparameter", "enable_custom_all_reduce", 0),
            model_dir,
...
template<typename T>
Baichuan2TritonModel<T>::Baichuan2TritonModel(size_t      tensor_para_size,
    ...
    shared_weights_(std::vector<std::shared_ptr<ft::Baichuan2Weight<T>>>(ft::getDeviceCount())),
    ...
    int8_mode_(int8_mode)
{
    ...
    model_name_           = reader.Get("baichuan2", "model_name");
    head_num_             = reader.GetInteger("baichuan2", "head_num");
    size_per_head_        = reader.GetInteger("baichuan2", "size_per_head");
    ...
    num_tasks_ = reader.GetInteger("baichuan2", "num_tasks", 0);

    prompt_learning_start_id_ = reader.GetInteger("baichuan2", "prompt_learning_start_id", end_id_ + 1);
    prompt_learning_type_ =
        static_cast<ft::PromptLearningType>(reader.GetInteger("baichuan2", "prompt_learning_type", 0));
...
template<typename T>
std::unique_ptr<AbstractTransformerModelInstance> Baichuan2TritonModel<T>::createModelInstance(
    int                                                               device_id,
    int                                                               rank,
    cudaStream_t                                                      stream,
    ...
    std::shared_ptr<ft::AbstractCustomComm>                           custom_all_reduce_comm)
{
    ...
    ft::AttentionType attention_type = ft::getAttentionType<T>(size_per_head_,
                                                               ...
                                                               true);  // causal_mask
    auto              gpt            = std::make_unique<ft::Baichuan2<T>>(
        ft::Baichuan2<T>(head_num_,
                     size_per_head_,
                     inter_size_,
                     num_layer_,
                     vocab_size_,
    ...
    return std::unique_ptr<Baichuan2TritonModelInstance<T>>(
        new Baichuan2TritonModelInstance<T>(std::move(gpt),
...
template<typename T>
void Baichuan2TritonModel<T>::createSharedWeights(int device_id, int rank)
{
    ft::check_cuda_error(cudaSetDevice(device_id));
    const int tensor_para_rank   = rank % tensor_para_size_;
    const int pipeline_para_rank = rank / tensor_para_size_;
    shared_weights_[device_id]   = std::make_shared<ft::Baichuan2Weight<T>>(head_num_ * size_per_head_,
                                                                        inter_size_,
                                                                        vocab_size_,
                                                                        num_layer_,
...
template<typename T>
std::string Baichuan2TritonModel<T>::toString()
{
    std::stringstream ss;
    ss << "Model: "
...
template<typename T>
void Baichuan2TritonModel<T>::createCustomComms(
...
template<typename T>
int Baichuan2TritonModel<T>::getTensorParaSize()
{
    return tensor_para_size_;
}

template<typename T>
int Baichuan2TritonModel<T>::getPipelineParaSize()
{
    return pipeline_para_size_;
}

template struct Baichuan2TritonModel<float>;
template struct Baichuan2TritonModel<half>;
#ifdef ENABLE_BF16
template class Baichuan2TritonModel<__nv_bfloat16>;
#endif
