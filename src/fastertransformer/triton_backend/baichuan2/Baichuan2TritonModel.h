...
template<typename T>
struct Baichuan2TritonModel: public AbstractTransformerModel {
    Baichuan2TritonModel(size_t      tensor_para_size,
                     size_t      pipeline_para_size,
                     int         enable_custom_all_reduce,
                     std::string model_dir,
                     int         int8_mode);

    ~Baichuan2TritonModel() = default;
...
private:
    ...
    std::vector<std::shared_ptr<ft::Baichuan2Weight<T>>> shared_weights_;
